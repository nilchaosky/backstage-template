...
// Login 用户登录（redisClient 作为参数传入）
func (s *Service) Login(ctx context.Context, redisClient *redis.Client, req *request.LoginRequest) (*response.LoginResponse, error) {
	// 根据用户名查询用户
	...
	if err := redisClient.SaveToken(ctx, userIDStr, accessToken, refreshToken, duration, refreshDuration); err != nil {
...
// Logout 用户退出登录（redisClient 作为参数传入）
func (s *Service) Logout(ctx context.Context, redisClient *redis.Client, userID string) error {
	// 从缓存中删除 token
	if err := redisClient.DeleteToken(ctx, userID); err != nil {
